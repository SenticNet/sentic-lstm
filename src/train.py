...
if data_name == 'sentihood':
    ...
    data = joblib.load(input_dir+'/data_cpts_all.150.pkl')
    
    num_ways = 3

else:
    num_epoch=15
    data = joblib.load(input_dir+'/data_semeval_all.150.pkl')
...
sentic_values = torch.Tensor(data['sentic_values']).unsqueeze(1)
...
if args.mode == 'sentic-bi':
    ...
    if args.mode == 'distance':
        ...
        model = DistDMN_Model(num_classes,max_length,len(dicts['token2id']),embd\
              ,emb_dim = input_dim,hidden_dim=hidden_dim,lr=0.001,num_ways=num_ways)
    elif args.mode == 'global-attention':
        
        from biatt_lstm_model import BiAttLSTM_Model
        
        model = BiAttLSTM_Model(num_classes,max_length,len(dicts['token2id']),embd\
              ,emb_dim = input_dim,hidden_dim=hidden_dim,lr=0.001,num_ways=num_ways)
    elif args.mode == 'global-attention-avg':
        
        from biatt_avg_lstm_model import BiAvgLSTM_Model
        
        model = BiAvgLSTM_Model(num_classes,max_length,len(dicts['token2id']),embd\
              ,emb_dim = input_dim,hidden_dim=hidden_dim,lr=0.001,num_ways=num_ways)
