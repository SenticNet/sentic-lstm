...
class AvgLSTM_Model(nn.Module):
    ...
    def target_avg(self,hidden_outputs,targets):
        
        target_outputs  = (targets.unsqueeze(2).expand_as(hidden_outputs) * hidden_outputs).sum(1)
    ...
    def target_attention_forward(self, lstm_outputs, targets):
        ...
        targets = self.target_avg(lstm_outputs,targets)
